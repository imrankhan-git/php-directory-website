...
else
{
	...
	if ($count == 1) {
		$adminemail1 = $row['broker_email'];
	    $adminname1 = $row['broker_first_name'];
		$email_encoded = rtrim(strtr(base64_encode($adminemail1), '+/', '-_'), '=');
		
	    $passwordresetlink = "https://kb.com/broker_resetpassword.php?recovery=".$email_encoded;
		
		
		
		$from1 = "kb <noreply@kb.com>";
		$to1=$adminemail1;
		$subject1 = "kb:Forgot password recovery";
